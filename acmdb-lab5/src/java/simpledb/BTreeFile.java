...
public class BTreeFile implements DbFile {
    ...
    private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        // some code goes here
        if (pid.pgcateg() != BTreePageId.INTERNAL) return (BTreeLeafPage) getPage(tid, dirtypages, pid, perm);
        BTreeInternalPage page = (BTreeInternalPage) getPage(tid, dirtypages, pid, Permissions.READ_ONLY);
        BTreePageId toVisit = null;
        BTreeEntry entry = null;
        for (BTreeInternalPageIterator iterator = (BTreeInternalPageIterator) page.iterator(); iterator.hasNext(); ) {
            entry = iterator.next();
            if (f == null || entry.getKey().compare(Op.GREATER_THAN_OR_EQ, f)) {
                toVisit = entry.getLeftChild();
                break;
            }
        }
        if (toVisit == null) toVisit = entry.getRightChild();
        return findLeafPage(tid, dirtypages, toVisit, perm, f);
    }
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        ArrayList<Tuple> entries = new ArrayList<>();
        for (int i = numTuples / 2; i < numTuples; ++i) {
            tuple = it.next();
            entries.add(tuple);
        }
        tuple = it.next();
        for (Tuple bTreeTuple : entries) {
            page.deleteTuple(bTreeTuple);
            newLeaf.insertTuple(bTreeTuple);
        }
        Field boundKeyField = tuple.getField(keyField);
        BTreeEntry boundEntry = new BTreeEntry(boundKeyField, page.getId(), newLeaf.getId());

        // Update sibling of the leaf pages
        ...
        if (rightSiblingId != null) {
            ...
            rightPage.setLeftSiblingId(newLeaf.getId());
        }
        page.setRightSiblingId(newLeaf.getId());


        addParentEntryForSplitPages(tid, dirtypages, page, newLeaf, boundEntry);

        return boundKeyField.compare(Op.GREATER_THAN_OR_EQ, field) ? page : newLeaf;
    }
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        BTreeEntry boundEntry = it.next();
        for (BTreeEntry bTreeEntry : entries) {
            page.deleteKeyAndRightChild(bTreeEntry);
            newInternal.insertEntry(bTreeEntry);
        }
        page.deleteKeyAndRightChild(boundEntry);
        Field boundKeyField = boundEntry.getKey();
        boundEntry = new BTreeEntry(boundKeyField, page.getId(), newInternal.getId());
        updateParentPointers(tid, dirtypages, newInternal);
        updateParentPointers(tid, dirtypages, page);

        addParentEntryForSplitPages(tid, dirtypages, page, newInternal, boundEntry);
        return boundKeyField.compare(Op.GREATER_THAN_OR_EQ, field) ? page : newInternal;
    }

    private void addParentEntryForSplitPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePage leftPage,
                                             BTreePage rightPage, BTreeEntry boundEntry) throws DbException, IOException, TransactionAbortedException {
        Field boundKeyField = boundEntry.getKey();
        BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, leftPage.getParentId(), boundKeyField);
        parent.insertEntry(boundEntry);
        ...
        assert rightPage.getParentId().equals(parent.getId());
    }
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        int moveNum = (siblingTupleNum - tupleNum) / 2;
        Iterator<Tuple> it = isRightSibling ? sibling.iterator() : sibling.reverseIterator();
        Tuple toMoveTuple = null;
        ArrayList<Tuple> toMoveTupleList = new ArrayList<>();
        for (int i = 0; i < moveNum; ++i) {
            toMoveTuple = it.next();
            toMoveTupleList.add(toMoveTuple);
        }
        for (Tuple tuple : toMoveTupleList) {
            sibling.deleteTuple(tuple);
            page.insertTuple(tuple);
        }
        entry.setKey(isRightSibling ? page.reverseIterator().next().getField(keyField) : sibling.reverseIterator().next().getField(keyField) );
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        int toMoveNum = (siblingEntryNum - entryNum) / 2;
        BTreePageId leftPageLastChild = leftSibling.reverseIterator().next().getRightChild();
        BTreePageId rightPageFirstChild = page.iterator().next().getLeftChild();

        Iterator<BTreeEntry> it = leftSibling.reverseIterator();
        LinkedList<BTreeEntry> bTreeEntries = new LinkedList<>();
        for (int i = 0; i < toMoveNum; ++i) {
            BTreeEntry entry = it.next();
            bTreeEntries.add(entry);
        }


        BTreeEntry middleEntry = bTreeEntries.pollLast();

        Field newParentKey = middleEntry.getKey();
        Field oldParentKey = parentEntry.getKey();
        parentEntry.setKey(newParentKey);

        parent.updateEntry(parentEntry);
        page.insertEntry(new BTreeEntry(oldParentKey, leftPageLastChild, rightPageFirstChild));

        for (BTreeEntry entry : bTreeEntries) {
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        int toMoveNum = (siblingEntryNum - entryNum) / 2;
        BTreePageId leftPageLastChild = page.reverseIterator().next().getRightChild();
        BTreePageId rightPageFirstChild = rightSibling.iterator().next().getLeftChild();
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        LinkedList<Tuple> toMoveTuples = new LinkedList<>();
        while (it.hasNext()) {
            toMoveTuples.add(it.next());
        }
        for (Tuple tuple : toMoveTuples) {
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        LinkedList<BTreeEntry> toMoveEntries = new LinkedList<>();
        while (it.hasNext()) {
            toMoveEntries.add(it.next());
        }

        leftPage.insertEntry(new BTreeEntry(parentEntry.getKey(), leftPage.reverseIterator().next().getRightChild(), toMoveEntries.getFirst().getLeftChild()));
        for (BTreeEntry entry : toMoveEntries) {
